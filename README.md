...
- **議事録の自動生成（オプトイン）**: 文字起こしが終わると、ローカルの llama.cpp で議事録
  ...
  7B 4.4GB、既定は 7B）から選べ、未取得のモデルは選択時（または初回の生成時）に Hugging Face
  から自動ダウンロードします（whisper と同じ SHA-256 検証つき）。選択肢には所要時間とメモリの
  目安を添えています。**CPU で動くため、既定の 7B は 4 分の会議で 1 分弱・実行時 8GB 前後の
  メモリを使います**。長い会議はチャンクに分けた 2 段要約になり、そのぶん時間がかかります。
