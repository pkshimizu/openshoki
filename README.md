...
- **録音停止時の自動文字起こし（オプトイン）**: ローカルの whisper.cpp で各音源をオンデバイス
  ...
  再利用します（SHA-256 検証つき。進捗は設定画面に表示）。`config.toml` の `whisper_model_path`
  でモデルファイルを上書きしている間は、選び直してもダウンロードしません（そのファイルが使われる
  ため。設定画面の状態行にもその旨を表示します）。上書き先が同じフォルダのカタログファイルを
  指している場合だけは、その機能のウィンドウの「Download」から明示的に取得できます（落とすことが
  そのモデルを動かす唯一の手段のため）。通信はこのダウンロード（受信）のみで、
...
- **議事録の自動生成（オプトイン）**: 文字起こしが終わると、ローカルの llama.cpp で議事録
  ...
  ことがあります。生成した議事録は Recordings ウィンドウの Notes タブで読めます（下記）。
...
- **文字起こしの状態表示と再実行**: 一覧の各行に文字起こしの状態（前 / 実行中 / 完了 / 失敗）が
  ...
  失敗の記録はアプリ再起動で消えます。
- **議事録の表示と手動生成**: 右ペインの「Transcript」「Notes」タブで表示を切り替えられ、
  Notes では生成済みの議事録（`summary.md`）を見出しを強調して表示します。詳細ペインの
  「Write notes」（生成済みなら「Regenerate notes」）ボタンで手動生成でき、生成済みのセッションも
  やり直せます（モデルを変えた後の
  ...
  （生成が始まってからは取り消せません）。
- **ダウンロード中の選び直し**: 文字起こし用・議事録用のどちらでも、ダウンロード中に別のモデルを
  選び直すと**前のダウンロードは打ち切られます**（受信済みのぶんは破棄され、再開はしません。
  中途半端なファイルも残りません）。ただし文字起こし・議事録生成のジョブが進行中で、そのジョブが
  そのモデルを使う場合は打ち切りません（ジョブは開始時点のモデルを使うため）。打ち切りは失敗とは
  区別され、状態は「未取得」に戻ります。
- **モデルの管理（一覧・取得・削除・選択）**: 設定画面の扉（`Transcription` / `Meeting notes`）から
  開く**機能ごとのウィンドウ**で、その機能が使うモデルを一覧できます。カタログ全件が並ぶので、
  まだ取得していないモデルも見えます。行ごとに表示名・説明・サイズ・状態（未取得 / 取得中の進捗 /
  取得済み / 失敗）が出て、末尾に取得済みの合計使用量が出ます。
  - **使うモデルを選べます**（「Use」）。**選ぶ場所はこの一覧だけ**です（設定画面には選択 UI が
    ありません）。文字起こし用のモデルは、選んだ時点で未取得なら**取得も始まります**
...
│   ├── transcription-window.slint 文字起こしの設定とモデル一覧（Slint）
│   ├── minutes-window.slint   議事録の設定とモデル一覧（骨格は文字起こしと同じ）
│   ├── model-list.slint       モデル一覧（取得・削除・選択）。上の 2 ウィンドウが共有する
...
└── src/
    ...
    │   ├── models.rs     モデル一覧の合成・可否判定・走査・通知・選択（2 ウィンドウで共有）
│   ├── transcription.rs 文字起こしウィンドウの設定と扉の文言
│   └── minutes.rs    議事録ウィンドウの設定と扉の文言
...
- **設定画面の描画確認**: 監視アプリ（Watched apps）の注記やモデルの説明は折り返すため、
  行の高さが伸びてレイアウトが詰まります。ビルドやテストでは検出できないので、確認用
  バイナリで目視します（`tall` / `min` / `bottom` / `empty-apps` を引数で切り替えられます）。
