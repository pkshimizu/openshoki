...
- **議事録の自動生成（オプトイン）**: 文字起こしが終わると、ローカルの llama.cpp で議事録
  ...
  設定に追従します。使う LLM は設定画面で 2 種（Qwen2.5 Instruct Q4_K_M の 7B / 3B、既定は
  7B）から選べ、未取得のモデルは選択時（または初回の生成時）に Hugging Face から自動
  ダウンロードします（whisper と同じ SHA-256 検証つき）。選択肢には所要時間とメモリの目安を
  添えています。**CPU で動くため、既定の 7B は 4 分の会議で 1 分弱・実行時 8GB 前後のメモリを
  使います**。長い会議はチャンクに分けた 2 段要約になり、そのぶん時間がかかります。軽さを
  優先したい場合は 3B（2.0GB）を選ぶと速度とメモリが半分ほどになりますが、細部を取り違える
  ことがあります。生成した `summary.md` の表示は今後の対応です。
