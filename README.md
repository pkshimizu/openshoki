...
- **録音停止時の自動文字起こし（オプトイン）**: ローカルの whisper.cpp で各音源をオンデバイス
  ...
  再利用します（SHA-256 検証つき。進捗は設定画面に表示）。ダウンロード中に別のモデルを選び直すと、
  前のダウンロードは打ち切られます（受信済みのぶんは破棄され、再開はしません）。ただし文字起こし・
  議事録生成のジョブが進行中で、そのジョブがそのモデルを使う場合は打ち切りません。
  `config.toml` の `whisper_model_path` で
  モデルファイルを上書きしている間は、選び直してもダウンロードしません（そのファイルが使われる
