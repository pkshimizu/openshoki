...
- **議事録の自動生成（オプトイン）**: 文字起こしが終わると、ローカルの llama.cpp で議事録
  ...
  状態でモデルを選び直した時点か、初回の生成時（Library ウィンドウからの手動生成を含む）
  ...
  ことがあります。生成した議事録は Library ウィンドウの Notes タブで読めます（下記）。
- **録音の一覧と再生**: メニューの「Library…」から、録音済みセッションを新しい順に一覧し、
  ...
  **macOS では、一覧を開くときに退避された録音の実体を取り寄せません**——iCloud などクラウドへ
  退避されている録音でも、ダウンロードを待たずにすぐ開きます（長さの見積もりは音源の先頭数バイトを
  読みますが、退避されているものはそこで諦めます）。その録音は長さの表示だけ省かれ、再生や
  文字起こしで実体が要るときに取り寄せられます。他の OS では、この抑止は効きません
  （macOS 固有の I/O ポリシーを使っているため）。
...
- **文字起こしの状態表示と再実行**: 一覧の各行に文字起こしの状態（前 / 実行中 / 完了 / 失敗）が
  ...
  失敗の記録はアプリ再起動で消えます。
- **途中で失敗しても、そこまでは残る**: 音源を最後まで読めなかったときも、読めた範囲を
  文字起こしして保存します。失敗の理由には「どこまで読めたか」が出て、「Show partial」で
  そこまでの文字起こしを読めます。**途中結果からは議事録を自動生成しません**（欠けたまま
  完成品に見える議事録ができるため）。**この区別はアプリを再起動しても残ります**——最後まで
  読めたかを文字起こしのファイルに記録するので、状態は「Transcribed in part」と表示され、
  議事録を書くときも欠けた入力であることが先に出ます。
- **中が抜けた文字起こしも、完成品と区別します**: 音声ファイルの一部が壊れていると、その
  部分は読み飛ばして続きを文字起こししますが、抜けたぶんの発話は残らず、以降の時刻も本来より
  早くなります。この場合は「Transcribed with gaps」と表示し、「途中で終わっている」とは
  別の説明を出します（やり直しで直るかは、原因が音声ファイルの破損か一時的な読み取り失敗かで
  変わるため、断定しません）。議事録は書けますが、欠けた入力であることが先に出ます。
- **文字起こしから議事録まで続けて実行**: 文字起こしが無い録音の Notes タブにある
  「Transcribe, then write notes」で、文字起こしと議事録を 1 回の操作で続けて実行できます。
  文字起こしが**全音源成功したときだけ**議事録へ進み、失敗したら Notes タブにそう表示されます。
  待っている間の状態（文字起こし中 → 順番待ち → 生成中）も Notes タブで追えます。
...
- **書きかけの一時ファイルの片付け**: 強制終了などで残った書きかけのファイル
  （`*.part.<番号>`）を、Library ウィンドウを開いたときに片付けます。対象はアプリが書く
...
│   ├── library-window.slint 録音一覧・再生ウィンドウの UI 定義（Slint）
